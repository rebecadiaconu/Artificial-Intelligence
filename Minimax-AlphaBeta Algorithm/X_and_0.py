...
class Joc:
    ...
    def mutari_joc(self, jucator):
        ...
        for i in range(len(mat_curent)):
            if mat_curent[i] == Joc.GOL:
                aux = [jucator if lin == i else mat_curent[lin] for lin in range(len(mat_curent))]
...
def main():
    ...
    raspuns_valid = False
    while not raspuns_valid:
        n = input("Adancime maxima a arborelui (de sugerat o valoare > 7): ")
        if n.isdigit():
